...
class ParticleFilter(Node):
    ...
    def load_map(self):
        # Loads the map from a CSV file. Ideally, this should use the package share directory.
    ...
    def odom_callback(self, msg):
        ...
        # Inverse motion model (Odometry sampled motion model)
        # Calculates relative motion steps: rot1, trans, rot2
    ...
    def crater_callback(self, msg):
        ...
        for i in range(self.num_particles):
            ...
            # Transforms all measurements to the global frame based on this particle hypothesis.
            # The crater detector outputs in the Camera Frame (x right, y down, z forward).
            # The Rover Base Frame is X forward, Y left, Z up.
            # Handles this frame transformation.
        ...
        if n_eff < self.num_particles / 2.0:
            ...
            self.weights.fill(1.0 / self.num_particles)
        # Else: Keeps the current particles and weights (Sequential Importance Sampling)
    ...
    def publish_particles(self):
        ...
        # Visualizes particles in the 'odom' frame assuming odom frame ~= map frame at start (0,0)
    ...
    def publish_estimated_pose(self, header):
        ...
        # Calculates a Q-Score (Match Quality) for the Mean Estimate.
        # This requires current observations 'z' which are not passed here, so logic is handled elsewhere or refactored.
        
    def calculate_q_score(self, mean_x, mean_y, mean_theta, obstacles):
        # Q-score logic based on Lunar Crater Navigation (Inverse of error)
        total_error = 0.0
        matches = 0
